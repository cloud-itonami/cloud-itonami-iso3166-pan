(ns marketentry.sim
  ...
  Art. 288 retail-trade restriction and its sector-conditional
  non-trigger for a non-retail sector."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (PAN, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up retail-trade-restriction) ==")
    ...
    (println "== filing/submit eng-4 (retail-trade + non-panamanian ownership -> HARD hold, Art. 288) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (non-retail sector, non-panamanian ownership -- Art. 288 must NOT fire) ==")
