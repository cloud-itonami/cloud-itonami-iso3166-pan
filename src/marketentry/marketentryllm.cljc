(ns marketentry.marketentryllm
  ...
  the PAN public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real PanamaCompra portal
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world PanamaCompra portal
  submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
     ...
     :rationale  (if e
                   (str "sector=" (:sector e)
                        " non-panamanian-ownership?=" (:non-panamanian-ownership? e)
     ...
     :confidence (if (and e
                          (not (and (= (:sector e) "retail-trade")
                                    (:non-panamanian-ownership? e))))
                   0.9 0.3)}))
