...
2. Register the client's intake: business type, target public function,
   prior filing history in Panama if any.
3. Run the advisor in read-only mode against PanamaCompra
   (panamacompra.gob.pa).
4. Compare the checklist against the client's current documentation
   (Registro Público entry).
...
- named referral relationship with Panamanian-licensed counsel or a
...
a working referral relationship with Panamanian-licensed counsel or a
registered agent for whatever licensed representation the law of
Panama requires for actual public-procurement filings.
